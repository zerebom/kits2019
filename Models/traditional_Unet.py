...
class UNet:
    def __init__(self, input_channel_count, output_channel_count, first_layer_filter_count, parser):
        ...
        enc1 = Conv2D(first_layer_filter_count, self.CONV_FILTER_SIZE, strides=self.CONV_STRIDE)(inputs)
        ...
        with tf.name_scope("dropout"):
            if use_dropout:
                enc5 = tf.keras.layers.Dropout(0.5, name='dropout')(enc5)



        # デコーダーの作成
        # (2 x 2 x 8N)
        dec1 = self._add_decoding_layer(filter_count, True, enc5)
        dec1 = concatenate([dec1, enc5], axis=self.CONCATENATE_AXIS)

        # (4 x 4 x 8N)
        filter_count = first_layer_filter_count * 8
        dec2 = self._add_decoding_layer(filter_count, True, dec1)
        dec2 = concatenate([dec2, enc4], axis=self.CONCATENATE_AXIS)
        ...
        dec4 = concatenate([dec4, enc2], axis=self.CONCATENATE_AXIS)



        # (256 x 256 x output_channel_count)
        dec5 = Activation(activation='relu')(dec4)
        ...
        if self.parser.insert_skip_inputs:
            dec5 = concatenate([dec5, inputs], axis=self.CONCATENATE_AXIS)
            dec5 = Conv2D(output_channel_count, kernel_size=3, padding='same', strides=1)(dec5)

        if self.parser.normalize_luminance:
            dec5 = Activation(activation='tanh')(dec5)

        self.UNET = Model(inputs, dec5)

    def _add_encoding_layer(self, filter_count, sequence):
        new_sequence = Conv2D(filter_count, self.CONV_FILTER_SIZE, strides=self.CONV_STRIDE)(sequence)
